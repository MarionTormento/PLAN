...
class custom_gripper():
    """docstring for custom_gripper"""
    def __init__(self, name):
        # super(custom_gripper, self).__init__()
        self.name = name
        self.pub = rospy.Publisher('/pan_controller/command', Float64, queue_size=10)

    def open(self):
        self.pub.publish(-0.4)
        time.sleep(5)
...
def graspAndApproach(collisionObject, arm):
    ...
    GRIP_SIZE = 0.13 # size of the open grippers
        ...
        if collisionObject.primitives[0].dimensions[1] <= GRIP_SIZE:        
            for quat in testPoses:
                ...
                if quat == right_side: 
                    ...
                    grasp.pose.position.y -= 0.03#+= min((collisionObject.primitives[0].dimensions[1]),0.05) #account for object being too tall most of the time
    ...
    # Box grasp is not fined tuned yet !
        ...
        if collisionObject.primitives[0].dimensions[0] <= GRIP_SIZE:
            ...
            euler_top = euler_from_quaternion(quat_top)
            angle_top = euler_top[0]+pi
            top_box.x = quat_top[0]
            top_box.y = quat_top[1]
            top_box.z = quat_top[2]
            top_box.w = quat_top[3]
            grasp_top.pose.position.z += collisionObject.primitives[0].dimensions[2]/2 + 0.02#account for object being too tall most of the time
            # grasp_side.pose.position.x += (collisionObject.primitives[0].dimensions[1]/3)*cos(angle_top) #= 0.93*grasp_side.pose.position.x
            # grasp_side.pose.position.y += (collisionObject.primitives[0].dimensions[1]/3)*sin(angle_top)#= 1.07*grasp_side.pose.position.y
            ...
            print angle_side, yaw
            print collisionObject.primitives[0].dimensions
            ...
            grasp_side.pose.position.x += (collisionObject.primitives[0].dimensions[1]/2-0.04)*cos(angle_side) #= 0.93*grasp_side.pose.position.x
            grasp_side.pose.position.y += (collisionObject.primitives[0].dimensions[1]/2-0.04)*sin(angle_side)#= 1.07*grasp_side.pose.position.y
        ...
        if collisionObject.primitives[0].dimensions[1] <= GRIP_SIZE:
            ...
            grasp_front.pose.position.x = 0.93*grasp_front.pose.position.x
            grasp_front.pose.position.y = 1.07*grasp_front.pose.position.y
...
if __name__=='__main__':
    try:
        ...
        for collisionObject in collisionObjects:
            ...
            picked, grasp = pick(name, collisionObject, 'right')
            if picked:
